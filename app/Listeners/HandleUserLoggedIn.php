    ...
    public function handle(UserLoggedIn $event)
    {
        ...
        if (strstr($_SERVER['HTTP_USER_AGENT'], 'iPhone') || strstr($_SERVER['HTTP_USER_AGENT'], 'iPad')) {
            Session::flash('warning',
                trans('texts.iphone_app_message', ['link' => link_to(NINJA_IOS_APP_URL, trans('texts.iphone_app'))]));
        } elseif (strstr($_SERVER['HTTP_USER_AGENT'], 'Android')) {
            Session::flash('warning', trans('texts.iphone_app_message',
                ['link' => link_to(NINJA_ANDROID_APP_URL, trans('texts.android_app'))]));
        }
        ...
        if (!Utils::isNinja()) {
            ...
            if (in_array(config('app.key'),
                ['SomeRandomString', 'SomeRandomStringSomeRandomString', 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'])) {
                ...
                Session::flash('error', trans('texts.mcrypt_warning',
                    ['command' => '<code>php artisan ninja:update-key --legacy=true</code>']));
            }
        }
    }
}
