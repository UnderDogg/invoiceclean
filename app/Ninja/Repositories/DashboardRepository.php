...
class DashboardRepository
{
    ...
    public function totals($accountId, $userId, $viewAll)
    {
        ...
        $select = DB::raw(
            'COUNT(DISTINCT CASE WHEN ' . DB::getQueryGrammar()->wrap('invoices.id',
                true) . ' IS NOT NULL THEN ' . DB::getQueryGrammar()->wrap('clients.id', true) . ' ELSE null END) billed_clients,
            SUM(CASE WHEN ' . DB::getQueryGrammar()->wrap('invoices.invoice_status_id',
                true) . ' >= ' . INVOICE_STATUS_SENT . ' THEN 1 ELSE 0 END) invoices_sent,
    ...
    public function paidToDate($account, $userId, $viewAll, $startDate = false)
    {
        $accountId = $account->id;
        $select = DB::raw(
            'SUM(' . DB::getQueryGrammar()->wrap('payments.amount',
                true) . ' - ' . DB::getQueryGrammar()->wrap('payments.refunded', true) . ') as value,'
        ...
        return $paidToDate->groupBy('payments.account_id')
            ->groupBy(DB::raw('CASE WHEN ' . DB::getQueryGrammar()->wrap('clients.currency_id',
                    true) . ' IS NULL THEN ' . ($account->currency_id ?: DEFAULT_CURRENCY) . ' ELSE ' . DB::getQueryGrammar()->wrap('clients.currency_id',
                    true) . ' END'))
            ->get();
    }
    ...
    public function averages($account, $userId, $viewAll)
    {
        ...
        return $averageInvoice->groupBy('accounts.id')
            ->groupBy(DB::raw('CASE WHEN ' . DB::getQueryGrammar()->wrap('clients.currency_id',
                    true) . ' IS NULL THEN CASE WHEN ' . DB::getQueryGrammar()->wrap('accounts.currency_id',
                    true) . ' IS NULL THEN 1 ELSE ' . DB::getQueryGrammar()->wrap('accounts.currency_id',
                    true) . ' END ELSE ' . DB::getQueryGrammar()->wrap('clients.currency_id', true) . ' END'))
            ->get();
    }
    ...
    public function pastDue($accountId, $userId, $viewAll)
    {
        ...
        return $pastDue->select([
            DB::raw("coalesce(invoices.partial_due_date, invoices.due_date) due_date"),
            'invoices.balance',
            'invoices.public_id',
    ...
    public function payments($accountId, $userId, $viewAll)
    {
        ...
        return $payments->select([
            ...
            DB::raw('(payments.amount - payments.refunded) as amount'),
            'invoices.public_id',
            'invoices.invoice_number',
            'clients.name as client_name',
            'contacts.email',
