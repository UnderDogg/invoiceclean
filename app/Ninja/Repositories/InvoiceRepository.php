...
class InvoiceRepository extends BaseRepository
{
    ...
    public function getClientDatatable($contactId, $entityType, $search)
    {
        $query = DB::table('invitations')
            ...
            ->whereRaw(sprintf("((accounts.account_key != '%s' and accounts.account_key not like '%s%%') or invoices.invoice_status_id = %d)",
                env('NINJA_LICENSE_ACCOUNT_KEY'), substr(NINJA_ACCOUNT_KEY, 0, 30), INVOICE_STATUS_PAID))
            ->select(
                DB::raw('COALESCE(clients.currency_id, accounts.currency_id) currency_id'),
                DB::raw('COALESCE(clients.country_id, accounts.country_id) country_id'),
                'invitations.invitation_key',
    ...
    public function findInvoiceByInvitation($invitationKey)
    {
        ...
        $invoice->load('user', 'invoice_items', 'documents', 'invoice_design', 'account.country', 'client.contacts',
            'client.country');
    ...
    public function save(array $data, Invoice $invoice = null)
    {
        ...
        */
        ...
        foreach ($data['invoice_items'] as $item) {
            ...
            // provide backwards compatability
            if (!isset($item['invoice_item_type_id']) && in_array($invoiceItem->notes,
                    [trans('texts.online_payment_surcharge'), trans('texts.online_payment_discount')])) {
